...
export async function loadHQ(): Promise<HQData> {
  ...
  const names = ['hq_accounts', 'hq_implementations', 'hq_tasks', 'hq_engagements', 'hq_events', 'audit_events'];
  ...
  if (results.slice(0, 5).some(r => r.data?.length === 1000)) throw new Error('This workspace needs pagination before more records can be displayed. Contact your administrator.');
  return { role, accounts: results[0].data, implementations: results[1].data, tasks: results[2].data,
    engagements: results[3].data, events: results[4].data, activity: results[5].data } as HQData;
}
