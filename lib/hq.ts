...
export const HQ_SERVICES = ['blueprint', 'implementation', 'migration', 'software', 'support'] as const;
export const HQ_SERVICE_LABELS = { blueprint: 'Blueprint', implementation: 'Implementation / custom build', migration: 'Data migration', software: 'Software subscription', support: 'Managed support' };
...
export type HQImplementation = { id: string; account_id: string; status: typeof HQ_DELIVERY[number]; owner: string; target_on: string | null; scope_snapshot: string; project_kind?: string; blocker: string; updated_at: string };
...
export type HQEngagement = { id: string; account_id: string; service: typeof HQ_SERVICES[number]; description: string; cadence: 'one_time' | 'monthly' | 'annual'; amount_cents: number; paid_cents: number; status: 'proposed' | 'agreed' | 'ended'; updated_at: string };
...
export function hqMetrics(data: Pick<HQData, 'accounts' | 'implementations' | 'tasks' | 'engagements'>, today: string) {
  ...
  return {
    ...
    monthlyCommitted: agreed.filter(e => e.cadence !== 'one_time').reduce((n, e) => n + e.amount_cents / (e.cadence === 'annual' ? 12 : 1), 0),
