...
export type PartnerLocation = { id:string; workspace_id:string; partner_id:string; name:string; address:string; city:string; region:string; postal_code:string; contact_name:string; phone:string; notes:string };
export type Partner = { id:string; name:string; kind:'crew'|'subcontractor'|'vendor'; phone:string; email:string; contact_name:string; trade:string; status:'active'|'hold'|'inactive'; coi_expires:string|null; w9:boolean; agreement:boolean; notes:string; primary_region:string; coverage_area:string; locations:PartnerLocation[] };
...
export type Purchase = { id:string; work_order_id:string; partner_id:string; description:string; cost_cents:number; status:'planned'|'ordered'|'received'; order_kind:'materials'|'subcontract'|'internal'; po_number:number; delivery_on:string|null; reference:string; notes:string };
