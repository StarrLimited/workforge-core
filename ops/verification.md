...
- Both database integration suites passed before and after applying the expansion, and again after the compatibility gate. Test data was rolled back. The existing Juniper workspace was activated on workflow version 2 after the deployed release and sign-in redirect were verified.
...
- Vercel's connected project read endpoint continues to return 404 for the confirmed working project. Both production and preview deployment attempts for 0.2 returned HTTP 403 permission errors. Direct deployment did not succeed. The subsequent Git connection deployed 0.2 successfully. The initial Git build omitted Supabase configuration; after the user saved Production/Preview variables, commit 7e0fe86b92291eb77b9286fa434b701c681930eb deployed successfully and the browser redirected from / to the working /login form. Canonical /api/version returned 0.2.0. See `release-0.2.md` and `resource-register.json`.
