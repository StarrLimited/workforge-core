...
import {PACKAGES,MONTHLY_OFFERS,FIELD_BASE_SCOPE,FIELD_SCOPE_RULES,THIRD_PARTY_TERMS,offerFor,buildPackageOffer} from '@/lib/hq-catalog';
...
export function PackageCatalog({items,canEstimate,onUse}:{items:HQPricebookItem[];canEstimate:boolean;onUse:(sku:string)=>void}){
  ...
  return <><div className="hq-section-heading"><h2>Field OS packages & pricing</h2></div>
    ...
    <div className="package-grid">{PACKAGES.map(pkg=><section className={'hq-panel package-card'+(pkg.sku==='WF-FIELD-OPERATIONS'?' recommended':'')} key={pkg.sku}>
      <div className="package-label">{pkg.sku==='WF-FIELD-OPERATIONS'?'Recommended for established teams':'BUILD & IMPLEMENTATION'}</div><h3>{pkg.name}</h3><strong className="package-price">{price(pkg.sku)}</strong><span>one time</span><p>{pkg.description}</p>
    ...
    <section className="hq-panel package-included"><h3>Included with every package</h3><ul>{FIELD_BASE_SCOPE.map(s=><li key={s}>{s}</li>)}</ul><p>{FIELD_SCOPE_RULES}</p></section>
...
export function PackageBuilder({items,initialSku='',onApply}:{items:HQPricebookItem[];initialSku?:string;onApply:(offer:ReturnType<typeof buildPackageOffer>)=>void}){
  const [sku,setSku]=useState(initialSku),[managed,setManaged]=useState(true),[automation,setAutomation]=useState(''),[ai,setAI]=useState(''),[improvement,setImprovement]=useState(false);
  const requiresManaged=!!automation||!!ai||improvement;
  const monthly=[...(managed||requiresManaged?['WF-MANAGED']:[]),...(automation?[automation]:[]),...(ai?[ai]:[]),...(improvement?['WF-IMPROVEMENT']:[])];
  ...
  return <section className="package-builder"><h3>Build from a Field OS package</h3><p>Choose an implementation and optional services. Applying fills the scope, fees and payment schedule; review the customer-specific terms below.</p>
    ...
    <label className="sales-checkbox"><input type="checkbox" checked={managed||requiresManaged} disabled={requiresManaged} onChange={e=>setManaged(e.target.checked)}/>{label('WF-MANAGED')}{requiresManaged?' (required for selected add-ons)':''}</label>
