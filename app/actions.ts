...
export async function mutate(workspaceId:string,action:string,form:FormData):Promise<ActionResult> {
  try {
    ...
    if(action==='demo_copy') {
      ...
      const keys=action==='qualification'?['service','source','sales_owner','priority','follow_up_on','sales_status','lost_reason','job_address']:action==='consultation'?['goals','measurements','access_notes','site_conditions','consultation_complete','consultation_summary']:['operations_owner','materials_status','walkthrough_complete','costs_reviewed','operations_notes'];
